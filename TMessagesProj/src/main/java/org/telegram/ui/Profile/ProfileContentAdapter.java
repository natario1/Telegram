...
public class ProfileContentAdapter extends RecyclerListView.SelectionAdapter {
    ...
    public boolean isEnabled(RecyclerView.ViewHolder holder) {
        ...
        if (hasNotification) {
            ...
            return kind == Rows.Notification || kind == Rows.Number || kind == Rows.Privacy
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        switch (viewType) {
            ...
            case VIEW_TYPE_USER: {
                boolean hasAddMember = rows.has(Rows.AddMember);
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, @SuppressLint("RecyclerView") int pos) {
        ...
        switch (holder.getItemViewType()) {
            case VIEW_TYPE_HEADER: {
                ...
                if (kind == Rows.InfoHeader) {
                    if (ChatObject.isChannel(fragment.getCurrentChat()) && !fragment.getCurrentChat().megagroup && rows.has(Rows.ChannelInfo)) {
            ...
            case VIEW_TYPE_ADDTOGROUP_INFO: {
                ...
                addToGroupInfo.setBackground(Theme.getThemedDrawable(addToGroupInfo.getContext(), R.drawable.greydivider, getThemedColor(Theme.key_windowBackgroundGrayShadow)));
                ...
                if (kind == Rows.Birthday) {
                    ...
                    if (userFull != null && userFull.birthday != null) {
                        ...
                        detailCell.setTextAndValue(
                                ...
                                fragment.getTopicId() != 0 || rows.has(Rows.BizHours) || rows.has(Rows.BizLocation)
                        );
                        containsGift = !fragment.isMyProfile() && today && !fragment.getMessagesController().premiumPurchaseBlocked();
                    }
                } else if (kind == Rows.Phone) {
                    String text;
                    ...
                    if (fragment.getCurrentUser() != null) {
                        ...
                        containsQr = true;
                    } else if (fragment.getCurrentChat() != null) {
                    ...
                    boolean divider = (fragment.getTopicId() != 0 || rows.has(Rows.BizHours) || rows.has(Rows.BizLocation)) && !rows.has(Rows.Birthday);
                    detailCell.setTextAndValue(text, alsoUsernamesString(username, usernames, value), divider);
                } else if (kind == Rows.Location) {
            ...
            case VIEW_TYPE_ABOUT_LINK: {
                ...
                if (kind == Rows.UserInfo) {
                    ...
                    aboutLinkCell.setText(text, ChatObject.isChannel(chat) && !chat.megagroup);
                } else if (kind == Rows.Bio) {
                    TLRPC.UserFull userFull = fragment.getMessagesController().getUserFull(fragment.getUserId());
                    String value;
            ...
            case VIEW_TYPE_CHANNEL: {
                ...
                channelCell.set(
                        fragment.getMessagesController().getChat(userInfo.personal_channel_id),
                        fragment.channelMessageFetcher != null ? fragment.channelMessageFetcher.messageObject : null
                );
                break;
            }
            case VIEW_TYPE_HOURS: {
            ...
            case VIEW_TYPE_SHADOW_TEXT: {
                ...
                if (kind == Rows.InfoSection) {
                    ...
                    if (botUser != null && botUser.bot && botUser.bot_can_edit) {
                        cell.setFixedSize(0);
                        cell.setText(LocaleController.formatString(R.string.ProfileBotAffiliateProgramInfoOwner, UserObject.getUserName(botUser), AffiliateProgramFragment.percents(userInfo != null && userInfo.starref_program != null ? userInfo.starref_program.commission_permille : 0)));
                    } else {
                        cell.setFixedSize(0);
            ...
            case VIEW_TYPE_TEXT: {
                ...
                } else if (kind == Rows.Subscribers) {
                    int text = ChatObject.isChannel(fragment.getCurrentChat()) && !fragment.getCurrentChat().megagroup ? R.string.ChannelSubscribers : R.string.ChannelMembers;
                    boolean divider = pos != rows.position(Rows.MembersSection) - 1;
                    TLRPC.ChatFull chatInfo = fragment.getMessagesController().getChatFull(fragment.getChatId());
                    if (chatInfo != null) {
                        textCell.setTextAndValueAndIcon(LocaleController.getString(text), LocaleController.formatNumber(chatInfo.participants_count, ','), R.drawable.msg_groups, divider);
                    } else {
                        textCell.setTextAndIcon(LocaleController.getString(text), R.drawable.msg_groups, divider);
                    }
                } else if (kind == Rows.SubscribersRequests) {
                    boolean divider = pos != rows.position(Rows.MembersSection) - 1;
                    TLRPC.ChatFull chatInfo = fragment.getMessagesController().getChatFull(fragment.getChatId());
                    if (chatInfo != null) {
                ...
                } else if (kind == Rows.Administrators) {
                    boolean divider = pos != rows.position(Rows.MembersSection) - 1;
                    TLRPC.ChatFull chatInfo = fragment.getMessagesController().getChatFull(fragment.getChatId());
                    if (chatInfo != null) {
                ...
                } else if (kind == Rows.BotStarsBalance) {
                    final BotStarsController controller = BotStarsController.getInstance(fragment.getCurrentAccount());
                ...
                } else if (kind == Rows.BotTonBalance) {
                    final BotStarsController controller = BotStarsController.getInstance(fragment.getCurrentAccount());
                ...
                } else if (kind == Rows.BlockedUsers) {
                    boolean divider = pos != rows.position(Rows.MembersSection) - 1;
                    TLRPC.ChatFull chatInfo = fragment.getMessagesController().getChatFull(fragment.getChatId());
                    if (chatInfo != null) {
                ...
                } else if (kind == Rows.AddToGroupButton) {
                    textCell.setTextAndIcon(LocaleController.getString(R.string.AddToGroupOrChannel), R.drawable.msg_groups_create, false);
                } else if (kind == Rows.Premium) {
                    Drawable drawable =  new AnimatedEmojiDrawable.WrapSizeDrawable(PremiumGradient.getInstance().premiumStarMenuDrawable, AndroidUtilities.dp(24), AndroidUtilities.dp(24));
                    textCell.setTextAndIcon(LocaleController.getString(R.string.TelegramPremium), drawable, true);
                    textCell.setImageLeft(23);
                } else if (kind == Rows.Stars) {
